 ...
 */
class Magegiant_Onestepcheckout_Block_Eav_Form extends Mage_Core_Block_Template
{
    /**
     ...
     * @return Magegiant_Onestepcheckout_Block_Eav_Form
     */
    ...
    protected function _prepareLayout()
    {
        if (empty($this->_xmlBlockName)) {
            Mage::throwException(Mage::helper('onestepcheckout')->__('Current module XML block name is undefined'));
        }
        if (empty($this->_formModelPath)) {
            Mage::throwException(Mage::helper('onestepcheckout')->__('Current module form model pathname is undefined'));
        }

        /* $var $template Magegiant_Onestepcheckout_Block_Eav_Form_Template */
     ...
     * @return array
     */
    public function getUserDefinedAttributesByCode($attribute_code)
    {
        $attributes = array();
        foreach ($this->getForm()->getUserAttributes() as $attribute) {
            if ($this->getExcludeFileAttributes() && in_array($attribute->getFrontendInput(), array('image', 'file'))) {
            ...
            if ($attribute->getIsVisible() && $attribute->getAttributeCode() == $attribute_code) {
                $attributes[$attribute->getAttributeCode()] = $attribute;
            }
        }
