 ...
 */
class Magegiant_Onestepcheckout_Adminhtml_Customer_FormtypeController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Load layout, set active menu and breadcrumbs
     *
     * @return Magegiant_Onestepcheckout_Adminhtml_Customer_FormtypeController
     */
    protected function _initAction()
    {
        $this->loadLayout()
            ->_setActiveMenu('customer/formtype')
            ->_addBreadcrumb(Mage::helper('onestepcheckout')->__('Customer'),
                Mage::helper('onestepcheckout')->__('Customer'))
            ->_addBreadcrumb(Mage::helper('onestepcheckout')->__('Manage Form Types'),
                Mage::helper('onestepcheckout')->__('Manage Form Types'));
        return $this;
    }
        ...
        if ($skeleton->getId()) {
            try {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('onestepcheckout')->__('An error occurred while saving the form type.'));
            }
            if ($hasError) {
    ...
    public function deleteAction()
    {
        ...
        if ($this->getRequest()->isPost() && $formType->getId()) {
            if ($formType->getIsSystem()) {
                $message = Mage::helper('onestepcheckout')->__('System form type cannot be deleted.');
                $this->_getSession()->addError($message);
            } else {
                try {
                    $formType->delete();
                    $message = Mage::helper('onestepcheckout')->__('Form type has been deleted.');
                    ...
                    $message = Mage::helper('onestepcheckout')->__('An error occurred while deleting the form type.');
