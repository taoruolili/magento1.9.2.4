 ...
 */
class Magegiant_Onestepcheckout_Adminhtml_Customer_AttributeController
    extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                ...
                Mage::helper('onestepcheckout')->__('Customer'))
            ->_addBreadcrumb(
                Mage::helper('onestepcheckout')->__('Manage Customer Attributes'),
                Mage::helper('onestepcheckout')->__('Manage Customer Attributes'));
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()
                    ->addError(Mage::helper('onestepcheckout')->__('Attribute is no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('onestepcheckout')->__('Edit Customer Attribute')
            : Mage::helper('onestepcheckout')->__('New Customer Attribute');
    ...
    {
        return Mage::helper('onestepcheckout/attribute_customer')->filterPostData($data);
    }
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $data) {
            ...
            try {
                Mage::dispatchEvent('onestepcheckout_attribute_before_save', array(
                    'attribute' => $attributeObject
                ));
                $attributeObject->save();
                Mage::dispatchEvent('onestepcheckout_attribute_after_save', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('onestepcheckout')->__('The customer attribute has been saved.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('onestepcheckout')->__('An error occurred while saving the customer attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            try {
                ...
                Mage::dispatchEvent('onestepcheckout_customer_attribute_delete', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('onestepcheckout')->__('The customer attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Mage_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('onestepcheckout')->__('An error occurred while deleting the customer attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('onestepcheckout/attributes/customer_attributes');
    }
}
